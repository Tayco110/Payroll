...
public class Functionalities_Employee {
    ...
    ///CHANGE EMPLOYEE///
    //change informations of an employee if hi's exists in the arraylist
    ...
    //add a service fee to an employee if he's exists in the arraylist and if he's are part of the union
    ...
    ///ADD SALE METHOD///
    //add a sale to an commissioned employee. only avaliable to this type.
    ...
    ///END ADD SALE METHOD///
    ///ADD TIMECARD///
    //add a timecard to an hourly employee. only avaliable to this type
    public void add_timecard(ArrayList<Hourly> hourlies, int id, int day) 
    {
        ...
        if(option.equals("check in"))
        {
            ...
            for(index=0;index<hourlies.size();index++)
            {
                ...
                {
                    finded = 1;
                    if((hourly.getIn_hour(day) == 0) && (hourly.getOut_hour(day) == 0))//check in pode ser feito se e somente se o empregado ainda n registrou entrada na empresa
                    {
        ...
        else if(option.equals("check out"))
        {
            ...
            for(index=0;index<hourlies.size();index++)
            {
                ...
                {
                    finded = 1;
                    if((hourly.getOut_hour(day) == 0) && (hourly.getIn_hour(day) != 0))//check out só pode ser feito se e somente se o empregado já registrou entrada
                    {
                        ...
                        if(hourly.getWorkedours(day) > 480)//calcula as horas trabalhadas e extras em minutos. 
                        {
